# Pong Remake - par PACKRYT

PONG Remake est une recréation fidèle du jeu de Pong classique, développé sur Scratch par PACKRYT.

Ce projet revisite l'un des tout premiers jeux vidéo en conservant son gameplay original tout en y ajoutant une touche de personnalisation pour les utilisateurs de Scratch.

### Gameplay :

Le jeu reprend les règles emblématiques de Pong :
Deux joueurs contrôlent chacun une raquette (barre) de chaque côté de l'écran.

Une balle rebondit entre les raquettes et les murs, et le but est d'empêcher la balle de passer derrière sa raquette.

Chaque fois qu'un joueur manque la balle, l'adversaire marque un point.

### Développement et fonctionnalités :

Développé sur Scratch , PONG Remake reste fidèle au style rétro du jeu original. L'expérience est entièrement classique , avec des contrôles simples et efficaces.


## Télécharger

### **PONG Remake est disponible partout, sauf sur appareil mobile.**

 PONG Remake sur mac --> [téléchargez ici](https://drive.google.com/uc?export=download&id=1i0wipvtOF7LjIcP9f__FTEyKncvAOR-5)

⊞ PONG Remake sur windows --> [téléchargez ici](https://drive.google.com/uc?export=download&id=1irhYipGEVypOjGeevq8xwwTfbrhyyIdg)


## Info

Google ne peut pas lancer d'analyse antivirus sur l'appli PONG Remake car cette application est soit disant "trop volumineux".
Au cas où vous aurait un message de ce genre, ne vous inquiéter pas, cette appli est sécurisée.
